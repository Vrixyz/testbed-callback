...
fn main() {
  ...
  ecs_world.borrow_mut().add_resource(PhysicsWorld(physics_world));
  let dispatcher = DispatcherBuilder::new()
        .with(DummySystem::new(), "dummy_system", &[])
        .build();

  let mut dispatcher = std::cell::RefCell::new(dispatcher);
  ...
  testbed.add_callback( move |_,_,_| {
    ...
    dispatcher.borrow_mut().dispatch(&mut ecs_world.res);
    ecs_world.maintain();
  });
  testbed.run();
}
